export default {
    all: 'Все',
    yes: 'Да',
    no: 'Нет',
    more: 'Подробнее',
    i_dont_know: 'Не знаю',
    you: 'Это вы',
    start: 'Начать',
    finish: 'Закончить',
    create: 'Создать',
    created: 'Создано',
    add: 'Добавить',
    added: 'Добавлен',
    not_added: 'Не добавлен',
    user_added: 'Добавлен(-а)',
    user_not_added: 'Не добавлен(-а)',
    copy: 'Скопировать',
    copied: 'Скопировано',
    like: 'Нравится',
    dislike: 'Не понравилось',
    delete: 'Удалить',
    instructions: 'Инструкции',
    note: 'Примечание',
    result: 'Результат',
    results: 'Результаты',
    verified: 'Проверено',
    not_verified: 'Не проверено',
    passed: 'Выполнено',
    your_choice: "Ваш выбор",
    choose: "Выбрать",
    choose_an_option: "Выберите вариант",
    choose_your_option: "Выберите свой вариант",
    choose_answer_option: "Выберите вариант ответа для каждой фразы или вопроса",
    download: "Скачать",
    loading: "Пожалуйста подождите, идет загрузка",
    export_to_png: "Экспорт в PNG",
    export_to_xlsx: "Экспорт в XLSX",
    export_to_pdf: "Экспорт в PDF",
    print_pdf: "Распечатать PDF",
    listen: "Слушать",
    all_rights: 'Все права защищены.',
    choose_your_language: 'Выберите Ваш язык',
    choose_a_mentor: 'Выберите учителя',
    choose_a_category: 'Выберите категорию',
    continue: 'Продолжить',
    merge: "Объединить",
    split: "Разделить",
    move_up: "Переместить вверх",
    move_down: "Переместить вниз",
    move_left: "Переместить влево",
    move_right: "Переместить вправо",
    save: 'Сохранить',
    saving: 'Сохранение',
    generate: "Cгенерировать",
    restart: 'Перезагрузить',
    edit: "Редактировать",
    welcome: 'добро пожаловать в EasySpeak!',
    you_are_on_the_main_page: 'Мы рады видеть вас на главной странице нашего сайта.',
    online: 'В сети',
    offline: 'Не в сети',
    ping: 'Пинг',
    or: 'Или',
    back: 'Назад',
    close: 'Закрыть',
    check: 'Проверить',
    swap_places: 'Поменять местами',
    an_error_occurred: 'Произошла ошибка',
    nothing_was_found_for_your_query: "По вашему запросу ничего не найдено",
    time_is_up: "Время вышло!",
    remaining_time: "Оставшееся время",
    ready: "Готово",
    already_started: "Уже началась",
    write_here_and_press_add: "Напишите здесь и затем нажмите на кнопку 'Добавить'",

    sort_by_this_column: 'Сортировать по данному столбцу',
    sort_asc: 'Сортировка по возрастанию',
    sort_desc: 'Сортировка по убыванию',

    status: "Статус",
    choose_a_status: "Выберите статус",
    invite: "Пригласить",
    search: "Искать",
    accept: "Принять",
    apply: "Применить",
    operator: "Оператор",
    iniciator: "Инициатор",
    executor: "Исполнитель",
    mentor: "Учитель",
    ai_mentor: "AI учитель",
    learner: "Ученик",
    applicant: "Заявитель",
    recipient: "Получатель",
    selected: "Выбрано",
    not_selected: "Не выбрано",
    not_specified: "Не указано",
    changed: "Изменено",
    your_answer: "Ваш ответ",
    learner_answer: "Ответ ученика",
    type_your_answer: "Введите ваш ответ",
    type_your_message: "Введите сообщение",
    no_answer: "Нет ответа",
    title: "Заголовок",
    text_center: "Текст по центру",
    right: "Правильно!",
    parthly_right: "Частично правильно",
    wrong: "Неправильно!",
    right_answer: "Правильный ответ",
    right_answers: "Правильные ответы",
    right_answers_count: "Правильных ответов",
    partly_correct_answer: "Частично правильный ответ",
    partly_correct_answers: "Частично правильные ответы",
    partly_correct_answers_count: "Частично правильных ответов",
    wrong_answer: "Неправильный ответ",
    wrong_answers: "Неправильные ответы",
    wrong_answers_count: "Неправильных ответов",
    unverified_answer: "Непроверенный ответ",
    unverified_answers: "Непроверенные ответы",
    unverified_answers_count: "Непроверенных ответов",
    answer_options: "Варианты ответа",
    add_answer_option: "Добавить вариант ответа",
    message: "Сообщение",
    write_a_comment: "Напишите комментарий",
    comment: "Комментарий",
    comments: "Комментарии",
    active: "Активных",
    content: "Содержание",
    for_re_examination: "Для повторного изучения",
    get_ready: "Приготовьтесь",
    thinking: "Думаю",
    recording: "Идет запись",
    recording_too_short: "Запись слишком короткая",
    recording_too_long: "Запись слишком длинная",
    recording_silent: "Запись слишком тихая",
    handling: "Обрабатывается",
    number_of_options: "Количество вариантов",
    number_of_attempts: "Количество попыток на исправление ошибок",
    random_order: "Выводить в случайном порядке",
    random_order_words: "Выводить слова в случайном порядке",
    random_order_pictures: "Выводить изображения в случайном порядке",

    created_at: "Дата создания",
    created_at_from: "Дата создания (с)",
    created_at_to: "Дата создания (по)",
    registered_at: "Дата регистрации",
    registered_at_from: "Дата регистрации (с)",
    registered_at_to: "Дата регистрации (по)",
    completed_at: "Дата выполнения",
    completed_at_from: "Дата выполнения (с)",
    completed_at_to: "Дата выполнения (по)",

    reset_search_filter: "Сбросить фильтр",
    show_search_filter: 'Показать фильтр поиска',
    hide_search_filter: 'Скрыть фильтр поиска',

    years: 'Годы',
    year: 'Год',
    month: 'Месяц',
    week: 'Неделя',
    day: 'День',

    start_time: 'Время начала',
    end_time: 'Время окончания',
    no_events: 'Нет мероприятии',
    add_to_calendar: 'Добавить в календарь',
    yet_another: 'Ещё',

    time: {
        millisecond: {
            title: 'Милисекунд',
            short: 'мс'
        },
        second: {
            title: 'Секунд',
            short: 'с'
        },
        minute: {
            title: 'Минут',
            short: 'м'
        },
        hour: {
            title: 'Час',
            short: 'ч'
        },
    },

    documents: 'Документы',
    check_your_data: 'Проверьте ваши данные',
    contacts: 'Контакты',
    requisites: 'Реквизиты',
    description: 'Описание',

    total: 'Итого',
    step: 'Шаг',
    step_num: "Шаг {num}",
    theme: {
        switch_to_a_light_theme: "Переключиться на светлую тему",
        switch_to_a_dark_theme: "Переключиться на темную тему"
    },

    save_changes: {
        title: "Сохранение изменении",
        confirm: "Вы хотите сохранить изменения?",
        save: "Да, сохранить",
        unsave: "Не сохранять",
        return: "Вернуться"
    },

    file: {
        file: "Файл",
        files: "Файлы",
        name: "Название файла",
        size: "Размер файла",
        type: "Тип файла",
        disk_info: "Информация о дисковом пространстве",
        show_disk_info: "Показать диск",
        hide_disk_info: "Скрыть диск",
        search_filter: "Фильтр поиска файлов",
        choose_a_file: "Выберите файл",
        choose_a_file_type: "Выберите тип файла",
        specify_the_file_name: "Укажите название файла",
        specify_the_file_type: "Укажите тип файла",
        selected_file: "Выбранный Вами файл",
        change_file: "Выбрать другой файл",
        file_ready_to_upload: "Файл готов к загрузке",
        upload: "Загрузить",
        upload_file: "Загрузить файл",
        uploading_a_new_file: "Загрузка нового файла",
        uploading_file: "Идет загрузка файла",
        upload_from_media: "Выбрать из ранее загруженных",
        generate: "Cгенерировать с помощью ИИ",
        deleting_a_file: "Удаление файла",
        file_delete_confirm: "Вы действительно хотите удалить данный файл? Все связанное с ним будет также удалено.",
        kilobyte: "КБ",
        kilobyte_sec: "кб/с",
        megabyte: "МБ",
        megabyte_sec: "мб/с",
        gigabyte: "ГБ",
        gigabyte_sec: "гб/с",
        total_space: "Общее пространство",
        free_space: "Свободно",
        used_space: "Занято",
        increase_the_space: "Увеличить место",
        lack_of_disk_space: "Не хватает места на диске для загрузки данного файла. Для увеличения места на диске Вам нужно перейти на более высокий тарифный план.",
        choose_file: "Выберите файл",
        replace: {
            title: "Замена файла",
            replace: "Заменить",
        },
        convert: "Преобразовать",

        image: {
            select: "Выберите файл изображения",
            current: "Текущий загруженный файл изображения"
        },
        audio: {
            select: "Выберите аудиофайл",
            current: "Текущий загруженный аудиофайл",
            loading: "Пожалуйста подождите, аудиофайл загружается",
            click_to_listen_to_the_audio: "Кликните чтобы вопроизвести аудио",
            click_to_stop_to_the_audio: "Кликните чтобы остановить воспроизведение аудио",
            voice: "Голос",
            choose_a_voice: "Выберите голос"
        },
        video: {
            select: "Выберите видеофайл",
            current: "Текущий загруженный видеофайл",
            player: {
                play: "Воспроизвести",
                pause: "Пауза",
                backward: "Воспроизвести заново",
                mute: "Без звука",
                unmute: "Со звуком",
                settings: "Настройки",
                speed: "Скорость",
                normal: "Нормальная",
                quality: "Качество",
                fullscreen: "На весь экран",
                exitFullscreen: "Выйти из полного экрана",
                processing: "Видеофайл обрабатывается. Пока вы смотрите оригинальный видеофайл"
            },
            markers: {
                title: 'Маркеры',
                add: 'Добавить маркер',
                name: 'Название маркера',
                name_error: 'Введите название маркера',
                second: 'Секунд',
                second_error: 'Введите секунду'
            }
        }
    },

    materials: {
        title: "Материалы",
        material: "Материал",
        add_material: "Добавить материал",
        edit_material: "Редактирование материала",
        delete_material: "Удаление материала",
        delete_confirm: "Вы действительно хотите удалить данный материал?",
        materials_count: "Материалов",
        sentence_materials: "Материалы фраз",
        annotation: 'Краткое описание (аннотация)',
        display: 'Показать ученикам',
        display_title: 'Показать материал ученикам',
        displayed: 'Ученики видят материал',
        displayed_title: 'Ученики видят данный материал',
        learn: {
            video: "Для ознакомления с материалом задания внимательно просмотрите данное видео",
            audio: "Для ознакомления с материалом задания внимательно прослушайте данное аудио",
            image: "Для ознакомления с материалом задания внимательно просмотрите данное изображение",
            text: "Для ознакомления с материалом задания внимательно прочитайте данный текст",
            table: "Для ознакомления с материалом задания внимательно проcмотрите данную таблицу"
        },
        show_materials_option: {
            use_both_options: "Использовать оба варианта",
            show_materials_during_a_task: "Показывать материалы во время выполнения задания",
            show_materials_before_starting_a_task: "Показывать материалы перед началом выполнения задания"
        },
        chat: {
            title: "Здравствуйте {name}! 👋 Чем я могу вам помочь?",
            material: {
                description: `👨‍🏫 Я — ваш виртуальный учитель и помощник в обучении. Моя задача — понятно и спокойно объяснять материал, который вы изучаете, помогать разбираться в сложных темах и отвечать на вопросы шаг за шагом.
                Если что-то кажется сложным — скажите, и я объясню по-другому. Давайте учиться вместе!😊`,
                prompts: {
                    explain: "Объясни материал",
                    example: "Приведи пример"
                }
            },
            speaking: {
                description: `Я ваш ИИ-учитель английского языка. Даже если вы никогда раньше не изучали английский, это не проблема — мы начнём с самых основ и будем двигаться шаг за шагом.
                
                ✨ Здесь нет сложных вопросов и неправильных ответов. Учитесь в своём темпе, задавайте любые вопросы и не бойтесь ошибаться — ошибки помогают учиться.
                
                Я помогу вам:
                • запоминать новые слова;
                • понимать английскую речь;
                • улучшать произношение;
                • тренировать грамматику;
                • практиковать общение на английском языке.
                
                Вы можете общаться со мной на русском языке, а когда будете готовы — постепенно переходить на английский.

                🚀Начнём ваше путешествие в мир английского языка?`,
                prompts: {
                    text_1: '👋 Давай познакомимся',
                    text_2: '🚀 Научи меня с нуля',
                    text_3: '🎯 Проверь мой уровень',
                    text_4: '💬 Давай поговорим на английском'
                }
            }
        }
    },

    text: {
        welcome: "Вы можете редактировать данный текст",
        bold: "Жирный",
        italic: "Курсивный",
        underline: "Подчеркнутый",
        strikethrough: "Зачеркнутый",
        title: "Заголовок",
        li: "Маркерный список",
        ol: "Нумерованный список",
        text_left: "Выравнивание по левому краю",
        text_center: "Выравнивание по центру",
        text_right: "Выравнивание по правому краю"
    },

    table: {
        create: "Создать таблицу",
        copy_and_paste_the_table_here: "Скопируйте таблицу и вставьте сюда",
        rows_count: "Количество строк",
        add_row: "Добавить строку",
        delete_rows: "Удалить строки",
        columns_count: "Количество столбцов",
        add_column: "Добавить столбец",
        delete_columns: "Удалить столбцы",
        delete_table: "Удалить таблицу",
        table_options: "Настройки таблицы",
        has_header: "Таблица с заголовками",
        is_small: "Компактная таблица",
        is_striped: "Полосатая таблица",
        is_bordered: "Таблица с границей",
        error_1: "Количество строк и столбцов должно быть выше 1",
        error_2: "Нельзя удалять все строки!",
        error_3: "Нельзя удалять все столбцы!",
        error_4: "Для начала Вам нужно создать таблицу",
        confirm_1: "Вы уверены, что хотите удалить таблицу?"
    },

    pagination: {
        first_page_title: "Перейти на первую страницу",
        previous_page_title: "Перейти на предыдущую страницу",
        next_page_title: "Перейти на следующую страницу",
        last_page_title: "Перейти на последнюю страницу",
        limit_per_page: "Лимит на страницу",
        unlimit: "Без ограничении",
        shown: "Показано: {shown} из {total}"
    },

    errors: {
        error_code: 'Код ошибки',
        page_is_not_available: "Эта страница недоступна для данной роли",
        show_error: "Показать ошибку",
        hide_error: "Скрыть ошибку",
        media: {
            camera_error: 'Не удается получить доступ к камере и микрофону.',
            screen_sharing_error: 'Демонстрация на этом устройстве недоступна либо ограничена.'
        },
        server: {
            error: 'Невозможно подключиться к серверу. Проверьте соединение с интернетом.',
            room_error: 'Не удается войти в конференцию. Попробуйте перезагрузить страницу.',
            peer_error: 'Не удается установить P2P соединение.  Проверьте соединение с интернетом.',
            user_network_error: 'У вас проблемы со связью, пытаемся восстановить...',
            other_user_network_error: 'У пользователя {name} есть проблемы со связью, пытаемся восстановить...',
            user_disconnect: 'Соединение потеряно.',
            other_user_disconnect: 'Соединение с пользователем {name} потеряно.'
        },
        statuses: {
            status_undefined: "Неизвестная ошибка!",
            status_400: "Непредвиденная ошибка на сервере!",
            status_401: "Вы не авторизованы!",
            status_403: "Доступ к данной странице запрещен!",
            status_404: "Запрашиваемая страница не найдена!",
            status_500: "Ошибка на стороне сервера!"
        }
    },

    form: {
        email: 'Эл.почта',
        school_email: 'Эл.почта школы',
        first_name: 'Имя',
        last_name: 'Фамилия',
        phone: 'Телефон',
        password: 'Пароль',
        new_password: 'Новый пароль',
        password_confirmation: 'Подтвердите пароль',
        new_password_confirmation: 'Подтвердите новый пароль',
        school_data: 'Данные школы',
        school_owner_data: 'Данные владельца школы',
        school_name: 'Название школы',
        full_school_name: 'Полное название школы (ИП/ТОО)',
        school_domain: 'Доменное имя школы',
        reset_school: 'Сменить школу',
        select_a_school: 'Выберите школу',
        schools_not_found: 'Нет школ в данном населенном пункте',
        country: 'Страна',
        region: 'Регион',
        city: 'Город',
        street: 'Улица, мкр-н',
        house: '№ дома, строения',
        flat: 'Квартира',
        bin: 'БИН/ИИН',
        iik: 'ИИК',
        bik: 'БИК',
        knp: 'КНП',
        fact_address: 'Фактический адрес',
        select_a_point: 'Выберите населенный пункт',
        select_an_internal_point: 'Выберите внутренний населенный пункт',

        card: {
            bank: 'Банк',
            input_bank_data: 'Введите данные банковской карты',
            card_num: 'Номер карты',
            expiration_at: 'Срок действия карты',
            month: 'Месяц',
            year: 'Год',
            cvv: '3 цифры с обратной стороны карты',
            name: 'Владелец карты',

            tiptop: {
                errors: {
                    'CardNumber_Empty': 'Введите номер карты',
                    'CardNumber_Invalid': 'Некорректный номер карты',
                    'Cvv_Empty': 'Введите CVV',
                    'Cvv_Invalid': 'Некорректный CVV',
                    'ExpDateMonthYear_Empty': 'Введите год и месяц',
                    'ExpDateMonthYear_Invalid': 'Некорректный год и месяц',
                    'ExpDateMonth_Empty': 'Введите месяц',
                    'ExpDateMonth_Invalid': 'Некорректный месяц',
                    'ExpDateYear_Empty': 'Введите год',
                    'ExpDateYear_Invalid': 'Некорректный год',
                    'Name_Empty': 'Пустое имя *',
                    'Name_Invalid': 'Некорректное имя *',
                    'Name_TooLong': 'Слишком длинное имя *',
                    'Name_TooShort': 'Слишком короткое имя *',
                }
            }
        }
    },

    telegram: {
        connect: {
            title: 'Подключиться к Telegram боту',
            connect: 'Подключить',
        },
        disconnect: {
            title: 'Отключение от Telegram бота',
            disconnect: 'Отключить',
            confirm: 'Ваш аккаунт {auth_user_name} будет отключен от Telegram аккаунта {tg_user_name}. После отвязки бота вы перестанете получать уведомления о занятиях и важных обновлениях. При необходимости вы сможете подключить бота снова в любое время. Вы уверены? Для продолжения нажмите кнопку "Отключить".',
        },
        banner: {
            title: 'Подключите Telegram-бот от EasySpeak',
            description: 'Будьте всегда в курсе событии, получайте уведомления о занятиях, смотрите расписание и отслеживайте свой прогресс прямо в Telegram.'
        },
        cancel: 'Отмена',
        message_1: 'Ваш аккаунт {auth_user_name} будет подключен к Telegram аккаунту {tg_user_name}. Для продолжения нажмите кнопку "Подключить".',
        message_2: 'Ваш аккаунт {auth_user_name} уже подключен к Telegram аккаунту {old_tg_user_name}. Для подключения к Telegram аккаунту {new_tg_user_name} нажмите кнопку "Подключить".'
    },

    promo: {
        code: 'Промокод',
        discount: 'Скидка по промокоду',
        desc_1: 'Введите промокод и получите скидку',
        errors: {
            not_found: 'Промокод с таким названием не найден',
            limit_has_expired: 'Лимит данного промокода исчерпан',
            date_expired: 'Истек срок действия данного промокода'
        }
    },

    board: {
        show_tool_parameters: 'Показать свойства инструмента',
        tool_parameters: 'Свойства инструмента',
        font_size: 'Размер шрифта',
        tools: {
            move: 'Перемещение',
            pencil: 'Карандаш',
            brush: 'Кисть',
            line: 'Линия',
            rectangle: 'Прямоугольник',
            circle: 'Круг',
            text: 'Текст',
            eraser: 'Ластик'
        },
        params: {
            text_color: 'Цвет текста',
            border_color: 'Цвет обводки',
            fill_color: 'Цвет заливки',
            cell_color: 'Цвет ячейки',
            border_width: 'Толщина линии',
            text_params: 'Параметры текста'
        },
        actions: {
            redo: 'Вернуть',
            undo: 'Отменить',
            clear: 'Очистить',
            save: 'Сохранить'
        }
    },

    weekdays: {
        monday: {
            title: 'Понедельник',
            even: 'Каждый понедельник'
        },
        tuesday: {
            title: 'Вторник',
            even: 'Каждый вторник'
        },
        wednesday: {
            title: 'Среда',
            even: 'Каждая среда'
        },
        thursday: {
            title: 'Четверг',
            even: 'Каждый четверг'
        },
        friday: {
            title: 'Пятница',
            even: 'Каждая пятница'
        },
        saturday: {
            title: 'Суббота',
            even: 'Каждая суббота'
        },
        sunday: {
            title: 'Воскресенье',
            even: 'Каждое воскресенье'
        },
    },

    pages: {
        error: {
            title: 'Произошла ошибка'
        },

        home: {
            title: 'Главная',
            'header-title': 'Цифровая образовательная платформа для изучения английского языка',
            hello: 'Добро пожаловать',
            to: "На главную страницу",
            description: 'Цифровая образовательная платформа для изучения английского языка',

            hero: {
                title: 'Платформа для изучения английского языка',
                description: 'Изучайте с нами или обучайте учеников по нашей методике. Готовое решение для онлайн-школ.',
                btn_1: 'Самостоятельное изучение с AI учителем',
                btn_2: 'Обучение с живым учителем от EasySpeak',
                btn_3: 'Регистрация школы партнера'
            },

            video: {
                title: 'Видео о платформе'
            },

            demo: {
                request_button: 'Запросить демонстрацию'
            },

            demo_modal: {
                title: 'Запрос на демонстрацию',
                description: 'Оставьте ваши контактные данные. В ближайшее время наш менеджер свяжется с вами для демонстрации платформы.',
                consent: {
                    text: "Я даю согласие на обработку моих персональных данных в соответствии c",
                    link: "политикой конфиденциальности",
                    text_2: "."
                },
                sended: '✅ Спасибо за интерес к нашей платформе! Наш менеджер свяжется с вами в ближайшее время.'
            },

            section_2: {
                card_1: {
                    title: 'Для учеников',
                    item_1: 'Личный кабинет с языковыми курсами, заданиями и прогрессом',
                    item_2: 'Доступ с любого устройства — ПК, планшета, телефона',
                    item_3: 'Чат с преподавателями и техподдержкой',
                    item_4: 'Мотивация: баллы, достижения, сертификаты',
                    item_5: 'Уроки в записи и прямом эфире'
                },
                card_2: {
                    title: 'Для школ и образовательных центров',
                    item_1: 'Курсы, уроки, тесты, видео — всё под контролем',
                    item_2: 'Расписание, оповещения и управление учениками',
                    item_3: 'Подключение преподавателей и настройка ролей',
                    item_4: 'Статистика, отчёты, оплата — всё в одном месте',
                }
            },
            section_3: {
                title: 'Кому подойдёт платформа?',
                card_1: {
                    title: 'Школам',
                    description: 'Брендированная онлайн-платформа с личными кабинетами учеников и преподавателей'
                },
                card_2: {
                    title: 'Преподавателям',
                    description: 'Инструменты для проведения уроков, контроля успеваемости и загрузки материалов'
                },
                card_3: {
                    title: 'Ученикам',
                    description: 'Удобный доступ к обучению, расписанию, домашним заданиям и прогрессу'
                },
            },

            section_4: {
                title: 'Некоторые скриншоты с нашего сайта',
                description: 'Өтінім қалдырыңыз, біз қалай оңай бастау керектігін көрсетеміз'
            },

            section_5: {
                title: 'Готовы запустить свою платформу?',
                description: 'Оставьте заявку, и мы покажем, как легко начать'
            }
        },

        "privacy-policy": {
            title: 'Политика конфиденциальности'
        },

        "refund-policy": {
            title: 'Политика возврата денежных средств'
        },

        "the-offer-agreement": {
            title: 'Договор публичной оферты'
        },

        profile: {
            title: 'Мой профиль',
        },

        auth: {
            title: 'Авторизация'
        },

        login: {
            title: 'Вход в систему',
            sign_in: 'Войти',
            dont_have_an_account: "Нет аккаунта?",
            forgot_password: "Забыли пароль?",
            password_recovery: "Восстановить",
            sign_out: 'Выйти из аккаунта',

            methods: {
                google: {
                    title: 'Войти через Google'
                },
                telegram: {
                    title: 'Войти через Telegram',
                    sdk_error: 'Telegram SDK пока еще не загружен. Повторите попытку позднее.'
                }
            }
        },

        logout: {
            title: 'Выход из аккаунта',
            please_wait: 'Пожалуйста подождите, идет выход из аккаунта'
        },

        register: {
            title: 'Регистрация',
            have_an_account: 'У меня есть акканут.',
        },

        "password-recovery": {
            title: 'Восстановление пароля',
            invalid: 'Ссылка для восстановления пароля уже недействительна. Попробуйте восстановить пароль заново.',
            send: 'Мы отправили письмо со ссылкой для восстановления пароля на указанный вами адрес электронной почты. Пожалуйста, проверьте ваш почтовый ящик, включая папку «Спам».'
        },

        "change-password": {
            title: 'Изменение пароля',
            inst_1: 'Здравствуйте {name}! Пожалуйста, придумайте новый пароль и подтвердите его. Убедитесь, что он надежный и содержит не менее 8 символов. После этого вы сможете снова войти в свой аккаунт.',
        },

        dashboard: {
            title: 'Личный кабинет',
            go_to_dashboard: 'Перейти в личный кабинет',
        },

        "users-groups": {
            title: 'Пользователи и группы',
        },

        users: {
            title: 'Пользователи',
            invite_user_title: "Приглашение нового пользователя",
            edit_user_title: "Редактирование данных пользователя",
            user_mode: 'Моя текущая роль',
            user_info: "Информация о пользователе",
            user_status: "Статус пользователя",
            user_role: "Роль пользователя",
            users_count: "Количество активных учеников",
            select_user_roles: "Выберите роль-(и) для нового пользователя",
            search_filter: "Фильтр поиска пользователей",
        },

        groups: {
            title: 'Группы',
            group: 'Группа',
            choose: 'Выберите группу',
            create_group: "Создать новую группу",
            saving_a_group: "Сохранение группы",
            create_group_title: "Создание новой группы",
            edit_group_title: "Редактирование группы",
            group_name: "Название группы",
            group_category: "Категория группы",
            group_description: "Описание группы",
            search_filter: "Фильтр поиска группы",
            add_members_to_group: "Добавление участников в группу",
            edit_group_members: "Редактирование участников",
            members: "Участники группы",
            members_count: "Количество участников",
            added_members_count: "Количество добавленных участников",
            note_1: "Чтобы добавить участников в группу кликните по ним в таблице ниже",
            warning_1: "Данный участник уже добавлен в группу",
            not_member: "Не состоит в группе",
            belongs_to_another_group: "Состоит в другой группе",
            already_members_of_a_similar_group: "Ученики ниже уже состоят в аналогичной группе такого же уровня. Удалите их из другой группы чтобы добавить их в эту группу."
        },

        school: {
            title: 'Школа'
        },

        training_types: {
            title: 'Форма изучения',
            select: 'Выберите форму изучения',
            types: {
                self_study: 'Самостоятельное изучение с помощью ИИ учителя',
                with_the_teacher: 'Изучение по видеоконференцсвязи с учителем'
            }
        },

        subscription: {
            plan: 'План подписки',
            your_plan: 'Ваш план подписки',
            expired: 'Для продолжения работы на платформе вам нужно продлить срок подписки',
            expired_at: 'Дата окончания подписки',
            prolongation_title: 'Продление подписки',
            prolongation: 'Продлить',
            choose_a_plan: 'Выберите срок подписки',
            months: "Месячная подписка | {n} месячная подписка",
            no_subscriptions: "Для данного курса пока не назначены подписки"
        },

        payment: {
            title: 'Просмотр оплаты',
            choose_a_payment_method: 'Выбор метода оплаты',
            choose_a_payment_form: 'Выберите форму приема оплаты',
            payment_method: 'Метод оплаты',
            payment_form: 'Форма приема оплаты',
            accept_payment: 'Подтверждение оплаты',
            please_wait: 'Пожалуйста подождите, выполняется прием оплаты',
            to_pay: 'Оплатить',
            to: 'Всего к оплате',
            making_an_order: 'Оформление заказа',
            check_order_data: 'Проверьте данные заказа',

            forms: {
                natural_person: 'От самого физ.лица (ученика)',
                legal_entity: 'От имени юр.лица',
                message_1: 'Внимание! Вы можете обрабатывать платежи только зарегистрированным группам от имени юридического лица.'
            },

            methods: {
                invoice: {
                    title: 'Счет на оплату',
                    invoices: 'Счета на оплату',
                    desc_1: 'Оплата выставленного счета через банк. Вам необходимо скачать счет в формате PDF.',
                    desc_2: 'Если вы оплачиваете счет в качестве физического лица, то вам нужно распечатать его и оплатить в кассе любого отделения любого удобного для вас банка.',
                    desc_3: 'Если вы оплачиваете счет в качестве юридического лица, передайте этот счет в электронном или распечатанном виде вашему бухгалтеру — он знает, что с ним делать.',

                    document: {
                        title: 'Платежное поручение',
                        num: '№ документа',
                        created_at: 'Дата составления',
                        payment_term: 'Срок оплаты',
                        beneficiary: 'Бенефициар (получатель)',
                        beneficiary_bank: 'Банк бенефициара (получателя)',
                        please_be_careful: 'Пожалуйста будьте внимательны!',
                        careful_desc: 'Счёт следует оплачивать от лица Плательщика, указанного в счёте. Оплата счёта третьими лицами не допускается. Если оплата будет произведена третьими лицами, денежные средства не будут зачислены на баланс лицевого счета, а следовательно услуги не будут активированы в автоматическом режиме.',
                        supplier: 'Поставщик',
                        executor: 'Исполнитель',
                        buyer: 'Покупатель',
                        customer: 'Заказчик',
                        footing: 'Основание',
                        without_contract: 'Без договора',
                        item: 'Наименование',
                        total_items: 'Всего наименовании',
                        item_num: 'Кол-во',
                        unit: 'Ед.',
                        price: 'Цена',
                        summ: 'Сумма',
                        summ_alt: 'на сумму',
                        piece: 'шт.',
                        total: 'Итого',
                        vat: 'НДС',
                        vat_text: 'В том числе НДС ({vat}%)',
                        download_pdf: 'Скачать счет в PDF формате'
                    },

                    accept: {
                        title: 'Принять оплату',
                        confirm: 'Вы действительно хотите принять данную оплату?'
                    }
                },
                card: {
                    title: 'Банковской картой'
                }
            }
        },

        'payment-result': {
            title: 'Результат оплаты',
            success: 'Оплата прошла успешно',
            success_alt: 'Оплачено',
            fail: 'Оплата не прошла',
            fail_alt: 'Не оплачено',
            fail_alt_2: 'Не оплачено',
        },

        '3ds': {
            title: '3-D Secure аутентификация',
            please_wait: 'Пожалуйста подождите, идет 3-D Secure аутентификация'
        },

        invoices: {
            title: 'Счета на оплату',
            search_filter: 'Фильтр поиска счетов',
            num: 'Счет №'
        },

        schedule: {
            title: 'Расписание уроков',
            show_schedule: 'Показать расписание',
            show_entire_schedule: 'Показать полное расписание',
            search_filter: 'Фильтр поиска по расписанию',
            lesson_days: 'Дни проведения уроков',
            select_lesson_days: 'Выберите дни проведения уроков',
            start_date: 'Дата начала расписания',
            start_date_from: 'Дата начала расписания (с)',
            start_date_to: 'Дата начала расписания (по)',
            end_date: 'Дата окончания расписания',
            start_time: 'Время начала урока',
            end_time: 'Время окончания урока',
            options: 'Настройки расписания',
            only_future: {
                title: 'Применить новое расписание только к будущим урокам',
                description: 'Прошедшие занятия останутся в истории без изменений. Новое расписание вступит в силу начиная с первого не пройденного урока.'
            },
            new_schedule: {
                title: 'Сформировать расписание заново',
                description: 'Новый график будет применён к курсу целиком (с самого начала). Даты всех уроков, включая уже прошедшие, будут сброшены и сформированы заново.'
            }
        },

        events: {
            title: 'Мероприятия',
            no_events: 'Никаких мероприятий на этот день не запланировано.'
        },

        conference: {
            title: 'Видеоконференция',
            create: 'Создать конференцию',
            create_conference: 'Создание конференции',
            delete_conference: 'Удаление конференции',
            delete_confirm: 'Вы действительно хотите удалить данную конференцию?',
            plan: 'Запланировать',
            plan_alt: 'Планирование конференции',
            mic: 'Микрофон',
            mic_turn_on: 'Включить микрофон',
            mic_turned_on: 'Микрофон включен',
            mic_turn_off: 'Отключить микрофон',
            mic_turned_off: 'Микрофон отключен',
            video: 'Видео',
            video_turn_on: 'Включить видео',
            video_turned_on: 'Видео включено',
            video_turn_off: 'Отключить видео',
            video_turned_off: 'Видео отключено',
            reflect_video: 'Отразить видео',
            video_reflected: 'Видео отражено',
            demo: 'Демо',
            demo_turn_on: 'Запустить демонстрацию экрана',
            demo_turn_off: 'Остановить демонстрацию экрана',
            board: 'Доска',
            participants: 'Участники',
            participants_count: 'Количество участников',
            chat: 'Сообщения',
            search_conference: 'Поиск конференции',
            join: 'Войти в конференцию',
            start_time: 'Дата и время начала конференции',
            until_the_start_of_the_conference: 'Осталось до начала конференции:',
            no_current_conferences: 'На данный момент нет текущих конференций.',
            not_appointed: 'Похоже, вы ещё не назначены учителем какой-либо группы. Пожалуйста, свяжитесь с администратором школы, чтобы решить этот вопрос. Если вы являетесь администратором, вам необходимо создать группу.',
            assign_a_mentor_only_for_this_lesson: 'Назначить учителем только на этот урок',
            appoint_a_mentor_for_this_and_subsequent_lessons: 'Назначить учителем на этот и последующие уроки',
            appoint_a_mentor_for_this_and_subsequent_lessons_note: 'Выбранный вами учитель будет закреплён за этим и последующими уроками данной группы.',
            you_cant_create_a_conference: 'Вы не можете создать новую конференцию, так как в данный момент уже проводится другая. Пожалуйста, дождитесь её завершения или удалите текущую конференцию.',
            not_bought: 'Чтобы принять участие в конференции вам нужно подписаться на один из этих курсов: {courses}.',
            free_conference: 'Бесплатная конференция (не проверять наличие подписки на курс)',
            mode: {
                grid: 'Сетка',
                slider: 'Карусель'
            },
            effects: {
                title: 'Фоновые эффекты',
                items: {
                    none: 'Без эффекта',
                    blur: 'Размытый',
                    image: 'Картинка'
                },
                images: {
                    title: 'Фоновые картинки',
                    items: {
                        bubbles: 'Пузырьки',
                        cafe: 'Кофейня',
                        city: 'Город',
                        clouds: 'Облака',
                        night: 'Ночь',
                        office: 'Офис',
                        palms: 'Пальмы',
                        retro: 'Ретро',
                        sakura: 'Сакура',
                        library: 'Библиотека',
                        cabinet: 'Кабинет',
                        flat: 'Квартира'
                    }
                }
            },
            messages: {
                requesting_access: 'Получение доступа к камере и микрофону',
                no_paid: 'Доступ к конференции ограничен, поскольку данный урок не был оплачен.',
                access_denied: 'Доступ к данной конференции разрешен только когда она бесплатна либо когда вы состоите в группе или были заранее записаны для участия в данной конференции.',
            },
            statuses: {
                current: 'Текущие',
                planned: 'Запланированные',
                passed: 'Пройденные',
            },
            leave: {
                title: 'Выход из конференции',
                confirm: 'Вы уверены, что хотите покинуть конференцию?',
                yes: 'Да, покинуть',
                no: 'Нет, остаться'
            },
            topic: {
                title: 'Тема конференции',
                description: 'Описание конференции'
            },
            poster: {
                title: 'Постер для конференции',
                required: 'Выберите постер для конференции'
            },
            accept: {
                title: 'Принять участие',
                success: {
                    title: 'Поздравляем! Вы успешно зарегистрированы на конференцию.',
                    description: 'Вы заняли свое место на конференции "{conf_name}". Ссылка на вход в конференцию появится в вашем личном кабинете за 10 минут до ее начала. Приготовьтесь прокачать свой язык! 🚀',
                    url_desc_1: 'Чтобы войти в конференцию',
                    url_desc_2: 'нажмите по данной ссылке'
                },
                already_exists: 'Вы уже записаны на эту конференцию',
                already_exists_alt: 'Вы записаны',
                limit_has_been_reached: 'Лимит участия в конференции исчерпан',
                free_limit_has_been_reached: 'Лимит участия в бесплатных конференциях исчерпан'
            }
        },

        "operations-requests": {
            title: 'Операции и запросы',
        },

        operations: {
            title: "Операции",
            operation_type: "Тип операции",
            search_filter: "Фильтр поиска операции",
            description: "Описание операции"
        },

        requests: {
            title: "Запросы",
            request_type: "Тип запроса",
            search_filter: "Фильтр поиска запросов",
            description: "Описание запроса"
        },

        notifications: {
            title: 'Уведомления',
            there_are_no_new_notifications: 'Новых уведомлений нет'
        },

        courses: {
            title: 'Курсы',
            my_courses: 'Мои курсы',
            course: 'Курс',
            go_to_courses: 'Перейти к курсам',
            choose_a_course: 'Выберите курс',
            course_level: 'Категория курса',
            choose_a_course_level: 'Выберите категорию курса',
            choose_a_course_levels: 'Выберите уровень(-ни) курса',
            level_overall_progress: 'Общий прогресс по уровню',
            section_overall_progress: 'Общий прогресс по главе',
            recommend_level: 'Рекомендуемый уровень',
            next_recommend_level: 'Отличный результат! Рекомендуем начать с',
            course_section: 'Глава',
            choose_a_course_section: 'Выберите главу',
            course_is_not_available: 'Курс недоступен. Пожалуйста, свяжитесь с администратором для получения доступа к курсу.',
            course_is_not_available_for_learner: 'Для получения доступа к курсу нужно добавить ученика в группу.',
            purchase: 'Пожалуйста, оформите подписку чтобы получить полный доступ ко всем урокам',
            has_expired: 'Срок вашей подписки истек, для того чтобы продолжить вам нужно оформить подписку на данный курс',
            purchase_button: 'Подписаться',
            not_sections: 'Нет секции для курса.',
            modal: {
                description: 'Пройдите короткий тест, чтобы узнать свой уровень владения языком! Для начала выберите школу и зарегистрируйтесь на нашей платформе.',
                description_alt: 'Пройдите короткий тест, чтобы узнать свой уровень владения языком.',
                sended: '✅ Спасибо за интерес к нашему курсу! Наш менеджер свяжется с вами в ближайшее время.'
            }
        },

        lessons: {
            title: 'Уроки',
            lessons_count: 'Уроков',
            lessons_list: 'Список уроков',
            there_is_no_lessons: 'Нет уроков',
            there_is_no_lessons_alt: 'Нет уроков для данной секции',
            pay_for_lessons: 'Оплатить занятия',
            pay_for_lessons_alt: 'Оплата занятии',
            price: 'Стоимость одного занятия',
            price_alt: 'Стоимость занятия',
            price_edit: 'Стоимость занятия может изменить только владелец школы',
            lesson: 'Урок',
            lesson_name: 'Название урока',
            free_lesson: 'Бесплатный урок',
            first_lesson_free: 'Первый урок бесплатный',
            all_lessons_is_conference: 'Все уроки курса проходят с учителем',
            bought_by_learner: 'Оплачено учеником',
            current_lessons: 'Текущие уроки',
            no_current_lessons: 'Пока нет текущих уроков на данный момент',
            gone_lessons: 'Пройденные уроки',
            upcoming_lessons: 'Ближайшие уроки',
            upcoming_lessons_alt: 'Предстоящие уроки',
            no_gone_lessons: 'Пока нет пройденных уроков на данный момент',
            no_upcoming_lessons: 'Пока нет ближайших уроков на данный момент',
            lesson_description: 'Краткое описание урока',
            choosing_a_lesson: 'Выбор урока',
            choose_a_lesson: 'Выберите урок',
            lesson_type: 'Тип урока',
            choose_a_lesson_type: 'Выберите тип урока',
            add_lesson: 'Добавить урок',
            add_unit: 'Добавить главу',
            add_unit_title: 'Добавление главы',
            unit_name: 'Название главы',
            lesson_materials: 'Материалы',
            there_are_no_materials: 'Для этого урока пока не добавлены материалы.',
            is_unavailable: 'Урок временно недоступен',
            is_unavailable_desc: 'Данный урок будет доступен после проведения конференции или контрольной работы с учителем.',
            start_date: 'Дата проведения урока (конференции)',
            start_time: 'Время проведения урока (конференции)',
            date_shift_couple: 'Сдвинуть дату проведения урока данной группы максимум на два календарных дня',
            date_shift_by_week: 'Сдвинуть дату проведения этого и последующих уроков данной группы относительно выбранной даты',
            date_shift_by_week_note: 'При выборе новой даты текущего урока система автоматически пересчитает даты всех последующих уроков в рамках этой группы, сохранив исходный интервал между ними. К примеру если уроки проводятся с интервалом в одну неделю, то каждая следующая дата будет смещена ровно на 7 дней от предыдущей, начиная с новой выбранной даты.',
        },

        "add-lesson": {
            title: 'Добавление урока'
        },

        "education-program": {
            title: 'Учебная программа',
            grade: 'Успеваемость ученика',
            get_grade: 'Проверить успеваемость'
        },

        quiz: {
            do_you_want_to_know_your_level: 'Хотите узнать свой уровень английского?',
            what_is_your_level: 'Как Вы думаете какой у Вас уровень владения английским?',
            recommend_route: 'Ваш рекомендуемый маршрут',

            marketing: {
                get_gift: 'Получить подарок',
                you_have_gift: 'Вы получили подарок',
                available_gift: "Вам доступны <b>{count} бесплатных онлайн-занятия в Speaking Club</b> с преподавателями EasySpeak.",
                choose_the_days: 'Выберите наиболее удобные для вас дни и время занятий, чтобы принять участие.',
                choose_the_one: 'Запишитесь хотя бы на один урок'
            }
        },

        dictionary: {
            title: 'Словарь',
            word: 'Cлово',
            count: 'Количество слов',
            options: 'Настройки словаря',
            this_word_is_hidden: 'Данное слово скрытое',
            hide_this_word: 'Скрыть это слово',
            this_word_is_extra: 'Данное слово указано лишним',
            make_this_word_extra: 'Указать это слово как лишний',
            select_words: 'Выбор слов',
            studied_words: 'Изученные слова',
            unstudied_words: 'Слова на переизучение',
            added_words_count: 'Количество добавленных слов',
            exactly_words_count_error: 'Количество добавленных слов должно составлять ровно {count}',
            min_words_count_error: 'Количество добавленных слов должно составлять как минимум {count}',
            add: 'Добавить словарь',
            add_section: 'Добавить секцию',
            add_these_words_to_the_section: 'Добавить эти слова в секцию',
            add_word_title: 'Добавление слова в словарь',
            words_left: "Осталось слов на повторение",
            edit_word_title: 'Редактирование слова',
            transcription: 'Транскрипция',
            seconds_per_word: 'Количество выделенных секунд на ответ каждого слова',
            required: 'Слово обязательно для заполнения',
            category_slug: 'Название категории на основном языке',
            there_are_no_words: 'Для этого урока пока не добавлено ни одно слово из общего словаря.',
            translate: {
                kk: 'Перевод слова на казахском языке',
                ru: 'Перевод слова на русском языке',
                category_kk: 'Название категории на казахском языке',
                category_ru: 'Название категории на русском языке',
            },
            search_filter: 'Фильтр поиска по словарю'
        },

        sentences: {
            title: 'Фразы',
            sentence: 'Фраза',
            select_sentences: 'Выбор фраз',
            studied_sentences: 'Изученные фразы',
            unstudied_sentences: 'Фразы на переизучение',
            added_sentences_count: 'Количество добавленных фраз',
            sentences_left: "Осталось фраз на повторение",
            add_sentence_title: 'Добавление фразы',
            edit_sentence_title: 'Редактирование фразы',
            transcription: 'Транскрипция',
            seconds_per_sentence: 'Количество выделенных секунд на ответ каждой фразы',
            required: 'Фраза обязательна для заполнения',
            translate: {
                kk: 'Перевод фразы на казахском языке',
                ru: 'Перевод фразы на русском языке'
            },
            search_filter: 'Фильтр поиска по фразам'
        },

        sections: {
            sections_count: 'Секции',
            added_sections_count: 'Количество добавленных секций',
            sections_left: "Осталось секций на повторение",
            studied_sections: "Изученные секции",
            unstudied_sections: 'Секции для переизучения',
            seconds_per_section: 'Количество выделенных секунд на ответ каждой секции',
        },

        questions: {
            title: 'Вопросы и ответы',
            question: {
                title: 'Вопрос',
                add: 'Добавить вопрос',
                delete: 'Удалить вопрос',
                required: 'Введите вопрос'
            },

            studied_questions: 'Изученные вопросы',
            unstudied_questions: 'Вопросы на переизучение',

            answer: {
                title: 'Ответ',
                delete: 'Удалить вариант ответа',
                required: 'Введите ответ',

                select_one: 'Выберите хотя бы один вариант ответа',
                select_count: 'Количество выбранных вариантов ответа должно быть не более {count}',

                select_one_alt: 'Выберите один из вариантов ответа',
                select_few: 'Выберите несколько вариантов ответов'
            },
            answers: 'Ответы',
            answered_questions: 'Отвеченные вопросы',
            unanswered_questions: 'Неотвеченные вопросы',
            number_of_answer_attempts: 'Количество попыток ответа на вопрос',
            added_questions_count: 'Количество добавленных вопросов',
            select_questions: 'Выбор вопросов',
            question_settings: 'Настройка вопросов',
            questions_left: "Осталось вопросов",
            seconds_per_question: 'Количество выделенных секунд на ответ каждого вопроса',
            predefined_answer: 'Предопределенный ответ',
        },

        tasks: {
            title: 'Задания',
            task_slug: 'Название задания на основном языке',
            task_name: 'Название задания',
            task_type: 'Тип задания',
            add_a_task: 'Добавить задание',
            adding_a_task: 'Добавление задания',
            delete_task: "Удаление задания",
            delete_confirm: "Вы действительно хотите удалить данное задание?",
            start_the_task: 'Начать задание',
            start_an_other_task: 'Запустить другое задание',
            select_a_task: 'Выбрать задание',
            select_a_task_alt: 'Выбор задания',
            complete_the_task: 'Завершить задание',
            start_the_test: 'Начать тестирование',
            test_is_started: 'Тестирование запущено',
            test_is_started_for_learners: 'Тестирование запущено. Ученики только что приступили к выполнению задании.',
            continue_the_test: 'Продолжить тестирование',
            finish_the_test: 'Завершить тестирование',
            find_out_the_result: 'Узнать результат',
            language_proficiency_test: 'Тест на определение уровня знаний языка',
            count: 'Количество задании',
            tasks_count: 'Задании',
            there_are_no_tasks: 'Для этого урока пока не добавлены задания.',
            not_started: {
                mentor: 'Вы еще не запустили ни одного задания для учеников',
                learner: 'Пока еще ни одно задание не было запущено учителем'
            },
            not_completed: {
                learner: 'Пока еще ни один ученик не выполнял данное задание'
            },
            task_materials: 'Материалы задания',
            try_again: 'Пройти заново',
            next_task: 'Следующее задание',
            run: {
                title: 'Запустить задание',
                self: 'Запустить для себя',
                learner: 'Запустить ученикам'
            },
            launched: {
                title: 'Задание уже запущено',
                learner: 'Уже запущено для учеников'
            },
            not_launched: 'Задание еще не запущено для выполнения',
            is_available: 'Задание уже доступно для выполнения',
            is_unavailable: 'Задание пока недоступно',
            allowed_only_participants: 'Доступ к заданиям разрешён только участникам этой конференции.',
            completed_the_task: '{name} выполнил(-а) задание "{taskName}".',
            new_task_is_available: 'Вам доступно задание "{taskName}". Вы можете открыть его внизу во вкладке "Задания".',
            completed_tasks: 'Количество выполненных задании',
            is_completed: 'Задание выполнено',
            in_process: 'Выполняет задание',
            in_process_this: 'Выполняет данное задание',
            is_being_reviewed: 'Задание на проверке',
            not_been_completed_yet: 'Задание еще не выполнено',
            show_task_result: 'Показать результат',
            show_learner_task_result: 'Просмотреть результат задания ученика',
            you_can_see_the_result: 'Вы можете увидеть результат задания ученика как только оно будет выполнено',
            your_result: 'Ваш результат',
            warning_1: 'Некоторые ученики все еще выполняют задание. Пожалуйста, дождитесь, пока они завершат выполнение, либо попросите их закрыть задание вручную.',
            task_options: {
                title: 'Настройки задания',
                task_example: 'Пример задания',
                task_example_message: 'Вы можете ввести здесь пример задания',
                show_task_example: 'Показывать пример задания',
                show_audio_button: 'Показывать аудиокнопку',
                play_audio_at_the_begin: 'Проигрывать аудио в начале задания',
                play_audio_with_the_correct_answer: 'Проигрывать аудио при правильном ответе',
                play_error_sound_with_the_incorrect_answer: 'Проигрывать звук ошибки при неправильном ответе',
                show_image: 'Показывать изображение',
                show_word: 'Показывать переводимое слово',
                show_transcription: 'Показывать транскрипцию',
                show_translate: 'Показывать перевод',
                impression_limit: 'Лимит показов',
                choose_one_of_the_methods_for_matching_words: 'Выберите хотя бы один из методов для сопоставления слов',
                choose_one_of_the_methods_for_matching_words_by_pictures: 'Выберите один из методов для сопоставления слов по изображениям',
                match_by_typing: 'Сопоставление с помощью ввода слова',
                match_by_clicking: 'Сопоставление с помощью клика',
                match_by_drag_and_drop: 'Сопоставление с помощью перетаскивания',
                option_labels: {
                    title: 'Метка вариантов ответа',
                    required: 'Выберите метку для вариантов ответа',
                    options: {
                        with_letters: 'Буквенная',
                        with_numbers: 'Цифровая',
                        hidden: 'Не показывать',
                    }
                },
                show_on_platform: {
                    title: 'На какой платформе показывать?',
                    options: {
                        b2b: 'Только на B2B платформе',
                        b2c: 'Только на B2C платформе',
                        both: 'Показывать в любом случае'
                    }
                }
            },
            translate: {
                kk: 'Название задания на казахском языке',
                ru: 'Название задания на русском языке',
                en: 'Название задания на английском языке',
                from_the_main_lang_to_the_current_lang: 'Перевод с основного языка на текущий язык сайта',
                from_the_current_lang_to_the_main_lang: 'Перевод с текущего языка сайта на основной язык'
            },
            missing_letters: {
                select_letters: 'Выбор букв',
                note_1: 'Выберите буквы, которые следует скрыть из слова.'
            },
            missing_words: {
                options: {
                    option_1: 'Заполнить пропуск во фразе (с подсказками)',
                    option_2: 'Заполнить пропуск во фразе (без подказок)',
                    option_3: 'Заполнить пропуск во фразе (с первой буквой)',
                    option_4: 'Заполнить пропуск во фразе (с одним из вариантов)'
                },
                note_1: 'Кликните на слово, которое нужно скрыть из фразы.'
            },
            match_words_by_pictures: {
                option_1: 'Сопоставление путем вписания слов в изображения',
                option_2: 'Сопоставление путем вписания порядковых номеров изображении',
            },
            find_an_extra_word: {
                note_1: 'Добавьте секцию. В секции создайте набор как минимум из трёх слов на основе словаря, одно из которых должно быть лишним. Например, среда, четверг, май, пятница (где "май" является лишним). По умолчанию лишнее слово не указано. Вы должны отметить слово как лишнее, кликнув на него. Допускается добавление неограниченного количества секций с наборами слов.',
                extra_words_count_error: 'Кликните по одному из слов в каждой секции, чтобы сделать его лишним'
            },
            match_same_words: {
                note_1: 'Добавьте секцию. В секции создайте набор как минимум из двух слов, где только первое слово будет видно, а остальные скрыты для сопоставления. Можно добавлять любое количество секций с такими наборами слов.',
                note_2: 'Выберите буквы, на которые падает ударение.'
            },
            right_or_wrong: {
                true: "Правда",
                false: "Ложь",
                "doesn't_say": "Нет упоминания",
                undefined: "Ответ не указан"
            },
            find_the_stressed_syllable: {
                note_1: 'Разделите каждое слово на слоги, затем выберите один слог в каждом слове и отметьте его как ударный.'
            },
            answer_the_questions: {
                answer_type: 'Выберите метод ответа на вопрос',
                paste_prohibited: 'Пожалуйста, напишите ответ самостоятельно. Вставка скопированного текста запрещена правилами тестирования.',
                options: {
                    option_1: 'Текстовый ответ',
                    option_2: 'Ответ с помощью видео',
                    option_3: 'Ответ с помощью аудио',
                },
            },
            check: {
                by_ai: 'Проверка ответа с помощью ИИ',
                by_mentor: 'Проверка ответа учителем',
            },
            search_filter: 'Фильтр поиска по заданиям'
        },

        'checking-tasks': {
            title: 'Проверка заданий'
        },

        training: {
            title: 'Тренировка',
            keyboard: {
                title: 'Вы также можете использовать клавиатуру для выбора букв',
                numbers: 'Вы также можете использовать цифры на клавиатуре для выбора ответа',
                warning: 'Вы должны поменять раскладку клавиатуры для соответствующего языка'
            },
            word_translation: {
                title: 'Проверим ваши знания',
                description: 'Найдите английский перевод этого слова'
            }
        },

        files: {
            title: 'Файловая библиотека'
        }
    }
}
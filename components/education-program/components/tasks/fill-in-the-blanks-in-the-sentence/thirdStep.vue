            ...
            <div class="card p-3">
                <div class="flex flex-col gap-y-3">
                    ...
                    <div class="btn-wrap items-center">
                        <div v-for="(word, wordIndex) in sentence.sentence.split(' ')" :key="wordIndex">
                            <button type="button" class="btn btn-light"
                                :class="sentence.removedWordIndex === wordIndex && 'disabled text-hidden'"
                    ...
                    <div class="flex flex-col gap-y-2"
                        v-if="findWordWithOptions === true && sentence.removedWordOptions && sentence.removedWordOptions.length > 0">
                        <p class="mb-0">{{ $t('answer_options') }}: <b>{{ sentence.removedWordOptions.length }}</b></p>

                        <div class="btn-wrap">
                            <tag v-for="(option, index) in sentence.removedWordOptions" :key="index" :itemId="index"
                                :itemLabel="option"
                                :onDelete="(optionIndex) => deleteFromOptions(sentenceIndex, optionIndex)"
                                :closable="index > 0"
                                :colorClass="sentence.removedWordOptions.length > 1 && index === 0 ? 'success' : ''"
                                v-motion="{
                                    initial: {
                                        y: -100,
                                        opacity: 0
                        ...
                        <div class="btn-wrap">
                            <input v-model="sentence.addOptionInput"
                                class="border rounded-lg px-2 text-sm duration-200 ease-in"
                                :class="sentence.addOptionInputError ? 'border-danger bg-danger placeholder-white wobble' : 'border-inactive'"
                                type="text" :title="$t('write_here_and_press_add')"
                                :placeholder="$t('write_here_and_press_add') + '...'" />
                            <button type="button" @click="addOption(sentenceIndex)"
                                class="btn btn-sm btn-outline-primary">
                                <i class="pi pi-plus-circle"></i> {{ $t('add') }}</button>
                        </div>
                    </div>
                </div>
            </div>
...
const { errors, selectedSentences, findWordWithOptions } = toRefs(props);
...
const toggleWordRemoval = (wordIndex, sentenceIndex) => {
    ...
    sentence.removedWordOptions.shift(); // Удаляем первый элемент в массиве
    sentence.removedWordOptions.unshift(sentence.sentence.split(' ')[wordIndex]); // Вставляем элемент а начало массива 
};

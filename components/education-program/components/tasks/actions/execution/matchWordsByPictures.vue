            ...
            <div class="custom-grid">
                            ...
                            <ul class="list-group nowrap">
                                <li v-for="(word, sIndex) in currentStudiedWords" :key="sIndex"
                                    ...
                                    <div class="flex items-center gap-x-2">
                                        <div v-if="word.image_file" :style="{
                                            backgroundImage: 'url(' + config.public.apiBase + '/media/' + word.image_file + ')',
                                        }" class="w-10 h-10 bg-cover bg-no-repeat bg-center"></div>
                                    ...
                                    <div class="flex flex-wrap gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-x-2">
                                                <div
                                                    class="btn btn-square btn-outline-success pointer-events-none font-medium">
                                                    {{ currentPictures.findIndex((p) => p.task_word_id === word.task_word_id) + 1 }}
                                                </div>
                ...
                <div v-else class="col-span-12">
                    <div class="custom-grid">
                            ...
                            <div class="custom-grid">
                                <div v-for="(picture, pictureIndex) in currentPictures" :key="pictureIndex"
                                    ...
                                    <img class="w-full p-3"
                                        :src="config.public.apiBase + '/media/' + picture.image_file" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-span-12 lg:col-span-6">
                            <ul class="list-group nowrap">
                                <li v-for="(word, wordIndex) in currentWords" :key="wordIndex" class="list-item">
                                    <div class="flex gap-3 items-center text-lg">
                                        <div @click="focusInput($event)"
                                            ...
                                            :class="(checkingStatus === true && (word.userInput === '' || word.userInput === ' ')) ? 'pulse btn-danger' : 'btn-outline-primary'">
                                            <input v-model="word.userInput" @input="changeFocus($event)" type="text"
                                                class="user_input"
                                                :style="{ width: (currentWords.length.toString().length + 0.5) + 'ch', textAlign: 'center' }"
...
const setWords = () => {
    ...
    if (words.value.length > 0) {
        ...
        currentPictures.value = [...currentWords.value].sort(() => Math.random() - 0.5);
...
const checkWords = () => {
    currentWords.value.forEach((word) => {
        ...
        if (/^\d+$/.test(word.userInput) && currentPictures.value[word.userInput - 1] && (word.task_word_id === currentPictures.value[word.userInput - 1].task_word_id)) {

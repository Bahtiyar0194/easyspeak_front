          ...
          <div class="flex flex-wrap justify-center items-center gap-2">
            ...
            >
              <audioPlayerWithWave
                :key="currentWord?.audio_file"
                :src="
                  config.public.apiBase + '/media/' + currentWord?.audio_file
                "
              />
            </div>
          </div>
...
const getTask = async () => {
  try {
    ...
    const res = await $axiosPlugin.get(
      "tasks/form_a_word_out_of_the_letters/" + props.task.task_id
    );
  ...
  if (words.value.length > 0) {
    ...
    if (Boolean(taskData.value.options.play_audio_at_the_begin)) {
      if (currentWord.value.audio_file) {
        stopAudio();
        playAudio(
          config.public.apiBase + "/media/" + currentWord.value.audio_file
        );
      }
    }

...
import firstStep from '../../missing-letters/firstStep.vue';
import secondStep from '../../missing-letters/secondStep.vue';
import thirdStep from '../../missing-letters/thirdStep.vue';
import fourthStep from '../../missing-letters/fourthStep.vue';
...
const newTaskSteps = [
    {
        title: t('pages.tasks.missing_letters.select_words'),
        component: firstStep,
...
const createTaskSubmit = async () => {
    ...
    await $axiosPlugin.post('tasks/create_missing_letters_task', formData)
        .then(res => {
            onPending(false);
            if (res.data.step) {

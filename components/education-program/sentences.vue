<template>
                        ...
                        <div class="custom-grid">
                            ...
                            <div class="col-span-12">
                                <div class="form-group-border active">
                                    ...
                                    <input type="date" name="created_at_from" @input="debounceSentences" placeholder=" " />
                            ...
                            <div class="col-span-12">
                                <div class="form-group-border active">
                                    <i class="pi pi-calendar"></i>
                                    <input type="date" name="created_at_to" @input="debounceSentences" placeholder=" " />
            ...
            <template v-if="sentences.data?.length > 0">
                <div class="table table-sm selectable">
                    ...
                    <table ref="tableRef">
                        <thead>
                            <tr>
                                <sortTableHead v-for="(head, index) in sentenceTableHeads" :key="index" :title="head.title"
                        ...
                        <tbody>
                            <tr v-for="sentence in sentences.data" :key="sentence.sentence_id" @click="getSentence(sentence.sentence_id)">
                ...
                <div class="btn-wrap mt-4">
                    <pagination :items="sentences" :setItems="getSentences" :onSelect="(count) => perPage = count" />
                    <client-only>
                        <tableToExcelButton :table="tableRef"
                            :fileName="$t('pages.sentences.title') + ' - ' + new Date().toLocaleString()"
                            :className="'btn-sm'" />
                    </client-only>
                </div>
            </template>
    ...
    <modal :show="addModalIsVisible" :onClose="() => closeAddModal()" :className="'modal-lg'" :showLoader="pendingAdd"
                ...
                <form @submit.prevent="addSentenceSubmit" ref="addFormRef">
                    <div class="custom-grid">
                        <div class="col-span-12">
                            <div class="form-group-border active">
                                <i class="pi pi-align-left"></i>
                                <input autoComplete="new-sentence" name="sentence" type="text" placeholder=" " />
                                <label :class="{ 'label-error': errors.sentence }">
                                    {{ errors.sentence ? errors.sentence[0] : $t("pages.sentences.sentence") }}
                                </label>
                            </div>
                        </div>
                        ...
                        <!-- <div class="col-span-12">
                            <div class="form-group-border active">
                                <i class="bi bi-braces"></i>
                                <input autoComplete="new-sentence-transcription" name="transcription" type="text"
                                    placeholder=" " />
                                <label :class="{ 'label-error': errors.transcription }">
                                    {{ errors.transcription ? errors.transcription[0] :
                                        $t("pages.sentences.transcription") }}
                                </label>
                            </div>
                        </div> -->
                        ...
                        <div class="col-span-12">
                            <div class="form-group-border active">
                                <i class="pi pi-align-left"></i>
                                <input autoComplete="new-sentence-kk-translate" name="sentence_kk" type="text"
                                    placeholder=" " />
                                <label :class="{ 'label-error': errors.sentence_kk }">
                                    {{ errors.sentence_kk ? errors.sentence_kk[0] : $t("pages.sentences.translate.kk") }}
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="col-span-12">
                            <div class="form-group-border active">
                                <i class="pi pi-align-left"></i>
                                <input autoComplete="new-sentence-ru-translate" name="sentence_ru" type="text"
                                    placeholder=" " />
                                <label :class="{ 'label-error': errors.sentence_ru }">
                                    {{ errors.sentence_ru ? errors.sentence_ru[0] : $t("pages.sentences.translate.ru") }}
                                </label>
                            </div>
                        </div>
    ...
    <modal :show="sentenceModalIsVisible" :onClose="() => sentenceModalIsVisible = false"
        :className="sentence?.image_file ? 'modal-3xl' : 'modal-lg'" :showLoader="pendingSentence" :closeOnClickSelf="true">
        <template v-slot:header_content>
            ...
            <div class="custom-grid">
                ...
                <div :class="sentence.image_file ? 'col-span-9 lg:col-span-8' : 'col-span-12'">
                    <div class="flex flex-col gap-y-3">
                        ...
                        <p class="mb-0"><span class="text-inactive">{{
                            $t("pages.courses.course") }}:</span> <b>{{ sentence.course_name }}</b></p>

                        <p v-for="(translate, index) in sentence.translates" :key="index" class="mb-0"><span
                                class="text-inactive">{{
                                    $t('pages.sentences.translate.' + translate.lang_tag) }}:</span> <b>{{
                                    translate.sentence_translate
                                }}</b></p>
                ...
                <form @submit.prevent="editSentenceSubmit" ref="editFormRef">
                    <div class="custom-grid">
                        <div class="col-span-12">
                            <div class="form-group-border active">
                                <i class="pi pi-align-left"></i>
                                <input autoComplete="eidt-sentence" v-model="sentence.sentence" name="sentence" type="text"
                                    placeholder=" " />
                                <label :class="{ 'label-error': errors.sentence }">
                                    {{ errors.sentence ? errors.sentence[0] : $t("pages.sentences.sentence") }}
                                </label>
                        ...
                        <!-- <div class="col-span-12">
                            <div class="form-group-border active">
                                <i class="bi bi-braces"></i>
                                <input autoComplete="edit-sentence-transcription" v-model="sentence.transcription"
                        ...
                        <div v-for="(translate, index) in sentence.translates" :key="index" class="col-span-12">
                            <div class="form-group-border active">
                                <i class="pi pi-align-left"></i>
                                <input :autocomplete="'edit-sentence-kk-translate-' + translate.lang_tag"
                                    v-model="translate.sentence_translate" :name="'sentence_' + translate.lang_tag" type="text"
                                    placeholder=" " />
                                <label :class="{ 'label-error': errors['sentence_' + translate.lang_tag] }">
                                    {{ errors['sentence_' + translate.lang_tag] ? errors['sentence_' + translate.lang_tag][0] :
                        ...
                        <div class="col-span-12">
                            <div class="form-group-border select active label-active">
                                ...
                                <select name="course_id">
                                    <option v-for="course in attributes.all_courses" :key="course.course_id"
                                        :value="course.course_id" :selected="sentence.course_id === course.course_id">
                                        {{ course.course_name }}</option>
                                </select>
                                <label :class="{ 'label-error': errors.course_id }">
                                    {{ errors.course_id ? errors.course_id[0] :
...
const current_sentence_image = ref(false);
...
const getSentence = async (sentence_id) => {
    ...
    await $axiosPlugin.get('sentences/get/' + sentence_id)
        .then(response => {
            errors.value = [];
...
const addSentenceSubmit = async () => {
    ...
    await $axiosPlugin.post('sentences/add', formData)
        .then(response => {
            getSentences();
            closeAddModal();
        }).catch(err => {

...
import secondStep from "../../right_or_wrong/secondStep.vue";
...
const editTaskSteps = [
  {
    ...
    props: { errors, selectedSentences, addAnswerType: true },
    modalSize: "full",
  },
  {
...
const editTaskSubmit = async () => {
  ...
  await $axiosPlugin
    .post("tasks/edit/right_or_wrong/" + props.task.task_id, formData)
    .then((res) => {
      onPending(false);
      if (res.data.step) {

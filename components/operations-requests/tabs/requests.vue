<template>
  <div class="custom-grid">
              ...
              <div class="col-span-12">
                <div class="form-group-border select active label-active">
                  ...
                  <select name="request_type_id" @change="debounceRequests">
                    ...
                    <option
                      v-for="request_type in attributes.request_types"
              ...
              >
                ...
                <td>
                  <div class="flex gap-x-2 items-center">
                    ...
                    />
                    {{ request.operator_last_name }}
                    {{ request.operator_first_name }}
                  </div>
                </td>
        ...
        <div class="btn-wrap mt-6">
          ...
          <client-only>
            <tableToExcelButton
              :table="tableRef"
              :fileName="
                $t('pages.requests.title') + ' - ' + new Date().toLocaleString()
              "
              :className="'btn-sm'"
            />
          </client-only>
    ...
    <modal
      ...
      :onClose="() => (requestModalIsVisible = false)"
      :className="'modal-lg'"
      :showLoader="pendingRequest"
      ...
      <template v-if="currentRequest" v-slot:body_content>
        <div class="flex flex-col gap-y-3">
          ...
          <p class="mb-0">
            ...
            >: <b>{{ new Date(currentRequest.created_at).toLocaleString() }}</b>
          </p>
...
const getRequest = async (user_request_id) => {
  ...
  await $axiosPlugin
    .get("requests/get/" + user_request_id)
    .then((response) => {
      errors.value = [];
...
const getRequestAttributes = async () => {
  pending.value = true;
  await $axiosPlugin
    .get("requests/get_request_attributes")
    .then((response) => {

<template>
  <div class="custom-grid">
            ...
            <div class="custom-grid">
              <div class="col-span-12">
                <multipleSelect
                  ...
                  :icon="'pi pi-file'"
                  :label="$t('pages.operations.operation_type')"
                  :items="attributes.operation_types"
                  :optionName="'operations[]'"
                  :optionValue="'operation_type_id'"
                  :optionLabel="'operation_type_name'"
        ...
        <div class="table table-striped table-sm selectable">
          ...
          <table ref="tableRef">
            <thead>
              <tr>
                <sortTableHead
                  v-for="(head, index) in operationsTableHeads"
                  :key="index"
                  :title="head.title"
                  :keyName="head.keyName"
                  :sortKey="sortKey"
                  :sortDirection="sortDirection"
                  :sortHandler="debounceOperations"
            ...
            <tbody>
              <tr
                v-for="(operation, index) in operations.data"
                :key="index"
              ...
              >
                ...
                <td>
                  <div class="flex gap-x-2 items-center">
                    ...
                    />
                    {{ operation.operator_last_name }}
                    {{ operation.operator_first_name }}
                  </div>
                </td>
    ...
    <modal
      :show="operationModalIsVisible"
      :onClose="() => (operationModalIsVisible = false)"
      :className="'modal-lg'"
      :showLoader="pendingOperation"
      ...
      <template v-if="currentOperation" v-slot:body_content>
        <div class="flex flex-col gap-y-3">
          ...
          <p class="mb-0">
            ...
            >:
            <b>{{ new Date(currentOperation.created_at).toLocaleString() }}</b>
          </p>
...
const getOperation = async (user_operation_id) => {
  ...
  await $axiosPlugin
    .get("operations/get/" + user_operation_id)
    .then((response) => {
      errors.value = [];
...
const getOperationAttributes = async () => {
  pending.value = true;
  await $axiosPlugin
    .get("operations/get_operation_attributes")
    .then((response) => {
